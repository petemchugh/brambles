...
Read.Header <- function(Header.File) {
  ...
  Header.File = "~/BrambleCloudLCM4/Watershed_Header_File.csv"
...
Read.Input.File <- function(header) {
  ...
  for (n.step in 1:N.input.files) {
    ...
    for (k in 1:K) {
      ...
      Watershed.Input.File = paste("~/BrambleCloudLCM4/",Watershed.Input.File,sep="")
    ...
    Cross.Site.Mig = read.csv(as.character(paste("~/BrambleCloudLCM4/",Cross.site.migration.file.names[n.step],sep="")), header=F,
