...
Read.Header <- function(Header.File) {
  ...
  Header.File = "~/BrambleCloudLCM8/Watershed_Header_File.csv"
...
Read.Input.File <- function(header) {
  ...
  for (n.step in 1:N.input.files) {
    ...
    for (k in 1:K) {
      ...
      Watershed.Input.File = paste("~/BrambleCloudLCM8/",Watershed.Input.File,sep="")
    ...
    Cross.Site.Mig = read.csv(as.character(paste("~/BrambleCloudLCM8/",Cross.site.migration.file.names[n.step],sep="")), header=F,
