...
AdultReturns = array(rep(0, K*Tr*G), c(K,Tr,G)) #Pete Apr 2016 supplementation tweak
...
#                         sum(CandN[]*Rel_Surv[k,i,t-1,]))	
      ...
      for(k in 1:K){
          for(g in 1:11){
            AdultReturns[k,t,g] = sum(NT[k,1:10,t,g])
        }
      }
      ...
      if(Hatch_Fish[1,15,t] == 2){ #Note, rule is set based on value in first subpop only
        ...
        if(TotAd > 0){
          ...
          for(i in 1:10){
            for(g in 1:11){
              AgeGCompTot[i,g]=sum(NT[,i,t,g])/TotAd #age-gtype comp of total return
              AgeGCompFem[i,g]=sum(NT_F[,i,t,g])/TotFem #age-gtype comp of total return
            }
          }
          ...
          if(TotAd<=sum(Inputs$Hatch_Fish[,16,t])){
            for(k in 1:K){
              for(i in 1:10){
                for(g in 1:11){
                  NT_F[k,i,t,g]=TotFem*AgeGCompFem[i,g]*Hatch_Fish[k,17,t]
                  NT[k,i,t,g]=TotAd*AgeGCompTot[i,g]*Hatch_Fish[k,17,t]
                }
              }
            }
          } else #move up to ceiling
          {
            for(k in 1:K){
              for(i in 1:10){
                for(g in 1:11){
                  NT_F[k,i,t,g]=(TotFem/TotAd)*Hatch_Fish[k,16,t]*AgeGCompFem[i,g]
                  NT[k,i,t,g]=Hatch_Fish[k,16,t]*AgeGCompTot[i,g]
                }
              }
            }
          }
...
#             #####################################################################################      
            ...
            for (g in 1:G) {
                  ...
                  #Pete Apr 2016, This is where mortality happens for prespawn (post accounting dam)
                  # BUT, this isn't the variable that determines eggs; that's below where Sr[k,2,t]
                  # actually affects eggs in gravel via p[k,1,t]
...
#####################################################################################
            ...
            # Pete Apr 2016, here is where prespawn mort gets implemented (as noted by Matt above)
            for (g in 1:G) {
